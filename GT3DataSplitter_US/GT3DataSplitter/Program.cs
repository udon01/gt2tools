...
namespace GT3.DataSplitter
{
    class Program
    {
        ...
        {
            IDStrings.Read(".id_db_idx_us.db", ".id_db_str_us.db");
            Strings.Read("paramstr_us.db");
            UnicodeStrings.Read("paramunistr_us.db");
            ColourStrings.Read("carcolor.sdb");

            var database = new ParamDB();
            database.ReadData("paramdb_us.db");
        ...
        static void BuildFile()
        {
            ...
            database.WriteData();
            IDStrings.Write();
            Strings.Write("paramstr_us.db", false);
            UnicodeStrings.Write("paramunistr_us.db", true);
            ColourStrings.Write("carcolor.sdb", true);
        }

        static void SplitFileGTC()
        {
            IDStrings.Read(".id_db_idx_gtc_us.db", ".id_db_str_gtc_us.db");
            Strings.Read("paramstr_gtc_us.db");
            UnicodeStrings.Read("paramunistr_gtc_us.db");
            ColourStrings.Read("carcolor.sdb");

            var database = new ParamDBConcept();
            database.ReadData("paramdb_gtc_us.db");
